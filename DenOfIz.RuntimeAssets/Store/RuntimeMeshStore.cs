    ...
    : IDisposable
{
    ...
    public RuntimeMeshHandle AddFromFile(string path, BatchResourceCopy batchCopy)
    {
        var loader = new MeshLoader();
        var result = loader.Load(path);

        if (!result.Success || result.Vertices == null || result.Indices == null)
        {
            return RuntimeMeshHandle.Invalid;
        }

        return AddRaw(result.Vertices, result.Indices, batchCopy, result.MeshType);
    }
    ...
    public RuntimeMeshHandle AddRaw(Vertex[] vertices, uint[] indices, BatchResourceCopy batchCopy, MeshType meshType = MeshType.Static)
    {
        ...
        var vertexSize = (uint)(vertices.Length * vertexStride);
        var indexSize = (uint)(indices.Length * sizeof(uint));

        const uint minAlignment = 256;
        var vertexAlignment = Math.Max(vertexStride, minAlignment);
