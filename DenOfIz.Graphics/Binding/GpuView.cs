...
public class GpuView : IDisposable
{
    ...
    private unsafe void BuildLightsFromScene(Scene scene)
    {
        ...
        {
            if (lightIndex >= MaxLights || !dl.IsActive) continue;
        ...
        {
            if (lightIndex >= MaxLights || !pl.IsActive) continue;
        ...
        // Add spot lights
        foreach (var sl in scene.SpotLights)
        {
            if (lightIndex >= MaxLights || !sl.IsActive) continue;
